...
##     @return true if the player has won, false otherwise.
    ...
    def checkForWin(self):
        ...
        for r in range(self.__max):
            if (board[r][0] == win_token) and (board[r][1] == win_token) and (
                    board[r][2] == win_token):
                win = True

        for c in range(self.__max):
            if (board[0][c] == win_token) and (board[1][c] == win_token) and (
                    board[2][c] == win_token):
                win = True

        if (board[0][0] == win_token) and (board[1][1]
                                           == win_token) and (board[2][2]
                                                              == win_token):
            win = True

        if (self.__game_board[0][2] == win_token) and (
                board[1][1] == win_token) and (board[2][0] == win_token):
            win = True
        return win
